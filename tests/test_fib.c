// This file simulates a task queue system where multiple threads (peers) execute tasks with different priorities.
// Task distribution: each processor gets equal number of tasks, but the tasks are distributed based on priority in a non-uniform way.
// Each task is a Fibonacci calculation with a random input. Each input is associated with a priority level (depending upon the defined range).
// The goal is to simualte uneven priority distribution and to ensure priority levels are respected.
...
int main()
{
  ...
  {
    // Each CPU gets a different number of tasks
    // The distribution is non-uniform based on the CPU ID to simulate uneven priority distribution
    // This is expected to perform more stealing which will help in gathering results
