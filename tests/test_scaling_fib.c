// This file divides the tasks based on the number of CPUs.
// This is done to observer how the implementation performs when the number of CPUs increases.
// Additionally, changing the total number of tasks will help in determining the overhead of smaller number of tasks when CPUs scale.
...
int main()
{
  ...
  for (int i = 0; i < num_cpu; ++i)
  {
    ...
    pthread_create(&tids[i], NULL, process_take, &peers[i]);
  }
