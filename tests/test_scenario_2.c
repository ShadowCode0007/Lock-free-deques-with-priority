// Scenario 2:
// Every CPU has equal number of tasks and every deque for each priority-level has equal number of tasks as well
// Tasks within each deque may take arbitrary amount of time to execute (simulated with sleep)
...
int main()
{
  ...
  for (int i = 0; i < num_cpu; ++i)
  {
    for (int j = 0; j < num_of_tasks; ++j)
    {
      ...
      if (i == 0)
        task.input = 50 + (rand() % (100));
      if (i == 1)
        task.input = 10 + (rand() % (100));
      if (i == 2)
        task.input = 60 + (rand() % (100));
      if (i >= 3)
        task.input = 20 + (rand() % (100));
  ...
  {
    peers[i].peers = peers;
    pthread_create(&tids[i], NULL, parallel_push_pop_take, &peers[i]);
  }
