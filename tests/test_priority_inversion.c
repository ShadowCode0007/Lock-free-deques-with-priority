// This file simulates a deviation from our work-stealing algorithm, where we do not check from all the CPUs before incrementing the priority.
// This is done to simulate a priority inversion scenario. This will perform better in terms of execution time as number of CPUs scale.
// However, there will be prirority inversions, thus leading to lower-priority tasks being executed prior to higher-priority tasks in some cases.
...
void *process_task(void *s)
{
  ...
  while (1)
  {
    ...
    if (task.priority == -1)
    {
      int check_cpus = (int)sqrt((double)data->num_peers);
...
int main()
{
  ...
  // Distribute equal number of tasks to each CPU
